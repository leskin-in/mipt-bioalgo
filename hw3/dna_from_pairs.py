...
def main():
    ...
    result = reconstruct_dna_from_pairs(pairs, k, d)
    print(result)
...
def reconstruct_dna_from_pairs(patterns: list, k: int, d: int) -> str:
    """
    Reconstruct the DNA from its paired reads (in the given list 'patterns').
    The reads are guaranteed to be of length 'k' and separated by 'd' other symbols
    """
    # A graph representation. Each vertex is a tuple: ([], ...); the list at 0 position is a list of connected vertices
    graph = []
    # Map of graph vertices (keys) to graph indexes
    graph_map = {}

    # Create a graph
    for pattern in patterns:
        pattern_key = pattern.split('|')

        v_from = (pattern_key[0][:-1], pattern_key[1][:-1])
        if v_from not in graph_map.keys():
            graph.append(([], v_from))
            graph_map[v_from] = len(graph) - 1
        v_from_index = graph_map[v_from]

        v_to = (pattern_key[0][1:], pattern_key[1][1:])
    ...
    euler_mers = [graph[euler_path[0]][1][0]]
    euler_last_mers = [None for _ in range(d + 2)]
    euler_last_mers[-1] = graph[euler_path[0]][1][1]
    for path_i in euler_path[1:]:
        current_vertex_mer = graph[path_i][1]
        euler_mers.append(current_vertex_mer[0][-1])
        euler_last_mers.pop(0)
        euler_last_mers.append(current_vertex_mer[1])
